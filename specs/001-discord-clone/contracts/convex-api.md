...
|---|---|---|---|---|
| `getMe` | query | `{}` | `User \| null` | Caller reads only their own profile |
| `updateProfile` | mutation | `{ name?: string, avatarStorageId?: Id<"_storage"> }` | `void` | Caller may only update their own profile row |
| `generateAvatarUploadUrl` | mutation | `{}` | `string` (upload URL) | Any authenticated user |
...
| Function | Type | Args | Returns | Auth rule |
|---|---|---|---|---|
| `getOrCreateForServer` | mutation | `{ serverId }` | `{ code: string }` | Caller must be the owner |
...
| Function | Type | Args | Returns | Auth rule |
|---|---|---|---|---|
| `list` | query (paginated) | `{ channelId, paginationOpts }` | `PaginationResult<Message>` | Caller must be a member of the channel's server |
...
| Function | Type | Args | Returns | Auth rule |
|---|---|---|---|---|
| `list` | query (paginated) | `{ threadId, paginationOpts }` | `PaginationResult<DirectMessage>` | Caller must be a participant in the thread |
...
| Function | Type | Args | Returns | Auth rule |
|---|---|---|---|---|
| `send` | mutation | `{ callId, toUserId, type, payload }` | `void` | Caller must be an active participant in `callId`; `toUserId` must also be an active participant |
| `listForMe` | query | `{ callId }` | `Signal[]` | Caller must be an active participant; returns rows addressed to caller |
| `ack` | mutation | `{ signalId }` | `void` | Caller must be the signal's `toUserId`; deletes the row |
